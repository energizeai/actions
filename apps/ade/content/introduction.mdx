...
const extractPersonInfoAction = createAction({
  ...
  .input({
    ...
    occupation: z.string().describe("The occupation of the person.").default("Unknown")
  })
...
- `getStockPriceAction` and `getRandomNumberAction` are the actions that we created earlier.

## Send the Actions to the LLM
...
async function invokeActions(userMessage: string) {
  ...
  const { results, toolCallMessages } = await toolCallsHandler(
  ...
  const secondResponse = await openai.chat.completions.create({
    ...
    messages: messages.concat(toolCallMessages), // add the tool calls to the messages
  })
